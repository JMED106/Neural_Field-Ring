...
from tools import Perturbation, qifint_noise, noise, SaveResults, TheoreticalComputations, DictToObj
...
while temps < d.tfinal:
    ...
    if d.system == 'qif' or d.system == 'both':
        ...
        if d.fp == 'noise':
            ...
            # Excitatory
            v, exit0, spikes = qifint_noise(v, exit0, spikes, d.eta0, s + p.input,
                                            noiseinput, temps, d.N,
                                            d.dN, d.dt, d.tau, d.vpeak, d.refr_tau, d.tau_peak)
        else:
            # Excitatory
            v, exit0, spikes = qifint.qifint(v, exit0, spikes, d.eta, s + p.input, temps, d.N,
                                             d.dN, d.dt, d.tau, d.vpeak, d.refr_tau, d.tau_peak)

        # Prepare spike matrices for Mean-Field computation and firing rate measure
        # Excitatory
        d.spikes_mod[:, (tstep + d.spiketime - 1) % d.spiketime] = 1 * spikes  # We store the spikes
        ...
        if not d.new_ic:
            # Voltage measure:
            vma = (exit0 <= temps)  # Neurons which are not in the refractory period
...
try:
    ...
    print "ERROR: don't erase v, exit0 and spikes"
    exit(-1)
